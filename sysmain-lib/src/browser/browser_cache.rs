...
///
/// This struct holds paths to browser cache directories and provides functionality
...
impl BrowserCache {
    ...
    ///
    /// # Arguments
    /// * `browser_name` - The browser type to create cache paths for, as a [`BrowserSelection`]
    ///
    /// # Returns
    /// A new `BrowserCache` instance with paths initialized for the specified browser
    ...
    ///
    /// Only attempts to clear directories that exist. For each existing cache directory,
    ...
    pub fn clear(&self) -> Result<(), std::io::Error> {
        for path in self.cache_paths.iter().filter(|p| p.exists()) {
            ...
            // Run the cache clearing operation in a closure, so we can log errors appropriately
            // TODO: Change result type to `crate::Result<()>`
            let ret: Result<(), std::io::Error> = (|| {
                ...
                entries
                    .filter_map(|e| {
                        e.inspect_err(|e| error!("Error reading directory entry: {e}"))
                            .ok()
                    })
                    .for_each(|entry| {
                        let path = entry.path();
    ...
    // TODO: Change result type to `crate::Result<()>`
    /// Processes a single entry in the cache directory, removing the directory or file.
