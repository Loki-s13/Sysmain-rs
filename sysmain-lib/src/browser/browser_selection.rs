...
/// An enum used to hold the different chromium prefix paths. (e.g Chrome, Chrome SxS, etc.)
...
impl ChromiumPrefix {
    /// Returns a vector of strings representing the prefix paths for the browser.
...
pub enum BrowserSelection {
    /// Chromium-based browsers, with specific prefix paths defined in [`ChromiumPrefix`]
...
impl BrowserSelection {
    /// Gets the base directory for the browser storage location, depending upon OS.
    ///
    /// Currently only supports Windows, using the LOCALAPPDATA environment variable.
    /// Returns a [`PathBuf`] pointing to the base storage directory.
    fn get_base_dir(&self) -> PathBuf {
        #[cfg(target_os = "windows")]
        {
            let home_dir =
                std::env::var("LOCALAPPDATA").expect("Failed to get APPDATA environment variable");
    ...
    ///
    /// For Chromium-based browsers, returns paths to both the main Cache and Code Cache directories
    /// for each browser prefix (e.g. Chrome, Chrome SxS, Edge).
    ///
    /// For Firefox, returns the path to the Profiles directory which contains cache data.
    ///
    /// # Returns
    /// A [`Vec<PathBuf>`] containing all relevant cache directory paths for the selected browser
    ...
    /// Creates a new [`BrowserSelection`] for a Chromium-based browser with the given prefix.
    pub fn create_chromium(prefix: ChromiumPrefix) -> Self {
        BrowserSelection::Chromium(prefix)
    }

    /// Creates a new [`BrowserSelection`] for the Mozilla Firefox browser.
    pub fn create_firefox() -> Self {
